...
// to run, use ./sim "11305_260" where Ex=11.305 MeV and Gamma = 0.260 MeV
int main(int argc, char *argv[])
{
  ...
  float thickness           = 0;     // target thickness in mg/cm^2
  ...
  frag[1] = new CFrag(1., Mass_p/m0, Loss_p_in_C, Loss_p_in_Si, CsiRes, thickness, distanceFromTarget, scale, useRealP);       // proton
	frag[2] = new CFrag(0., Mass_n/m0, Loss_n_in_C, Loss_n_in_Si, CsiRes, thickness, distanceFromTarget, scale, useRealP);       // neutron

	// Temporary fix for neutron not being reconstructed
	delete frag[2]->recon;
	frag[2]->recon = frag[2]->real;
  ...
  // and uses that to select a randomized value for phi and theta
	string Xsecfiles[nexits] = {
		string(XSECPATH) + "7li12c_e35_xsec_2.out",
		string(XSECPATH) + "7li12c_e35_xsec_3.out",
		string(XSECPATH) + "7li12c_e35_xsec_4.out",
  ...
  for (int index=0;index<Nevents;index++) {
    ...
    fragBeam->real->energy = Ebeam; //~5MeV/u Li-7
    ...
    output.protonenergy->Fill(frag[0]->recon->energy);
    output.alphaenergy->Fill(frag[1]->recon->energy);
    ...
    float x = frag[1]->recon->x/10.;
    float y = frag[1]->recon->y/10.;
    ...
    output.coreXY_S->Fill(x,y);
		...
		output.SetThetaNeut(frag[2]->real->theta*rad_to_deg);
		output.Fill();
  }

...
fit::fit(int n0, int NN0,int Npeaks0, double *para20, bool *useit0):minimizeND(n0)
{
  ...
  //TFile fileX("/home/Li7star/Li7/plotting/out_nosub.root");
  TFile fileX("/home/Li7star/Li7/sort_He6_CD2.root");
  TH1I* hist;

  hist = (TH1I*) fileX.Get("InvMass/7Li/Ex_7Li_p6He_timegate");
  //hist = (TH1I*) fileX.Get("Ex_7Li_p6He_timegate");
  hist->Rebin(2);
...
void fit::read2new(string fn1, string fn2)
{
  filename1 = fn1;
  filename2 = fn2;
  ...
  TFile * fsim2;
  fsim2 = new TFile (filename2.c_str());
  //fsim2 = new TFile ("/home/Li7star/Li7/plotting/out_sim_11690_200.root");
...
void fit::read3new(string fn1, string fn2, string fn3)
{
  filename1 = fn1;
  filename2 = fn2;
  filename3 = fn3;
  ...
  TFile * fsim3;
  fsim3 = new TFile (filename3.c_str());
  TH1I* sim3 = (TH1I*) fsim3->Get("Ex");
  effcorr(sim3);
  ...
  for (int j=low;j<high;j++)
  {
    ...
    Peak[0].y[i] = sim1->GetBinContent(j);
    Peak[1].y[i] = sim2->GetBinContent(j);
    Peak[2].y[i] = sim3->GetBinContent(j);
  }
...
void fit::readeff()
{
  ...
  TFile file_bkg("../rootout/sim_uniform.root");
...
void fit::effcorr(TH1I* h)
{
  ...
  for (int i=0;i<200;i++)
  {
    h->SetBinContent(i, h->GetBinContent(i)*eff[i]); 
  }
...
double fit::getBackground(int i, double *para3)
{
  ...
  //return (para3[nn]+para3[nn+1]*x[i] + para3[nn+2]*pow(x[i],2));
  return (para3[nn]+para3[nn+1]*(x[i]-10));
